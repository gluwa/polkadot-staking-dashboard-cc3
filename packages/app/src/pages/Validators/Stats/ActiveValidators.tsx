...
export const ActiveValidators = () => {
  ...
  const { sessionValidators } = useValidators()
  ...
  if (validatorCount > 0) {
    activeValidatorsAsPercent = new BigNumber(
      sessionValidators.length
  ...
  const params = {
    ...
    stat: {
      value: sessionValidators.length,
      total: validatorCount,
      unit: '',
    },
    pieValue: percentageOf(sessionValidators.length, validatorCount),
